...
export function BookPage() {
  ...
  const [routeEstimate, setRouteEstimate] = useState<{ durationSeconds: number | null; estimatedTotal: number } | null>(null);
...
function RoutePreview({ pickup, dropoff, pickupLatitude, pickupLongitude, dropoffLatitude, dropoffLongitude, onEstimate }: { pickup: string; dropoff: string; pickupLatitude?: number; pickupLongitude?: number; dropoffLatitude?: number; dropoffLongitude?: number; onEstimate: (estimate: { durationSeconds: number | null; estimatedTotal: number } | null) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch(apiUrl('/api/deliveries/route-preview'), {
        ...
        .then((route) => { if (!controller.signal.aborted) {
          setEncodedPolyline(typeof route?.encodedPolyline === 'string' ? route.encodedPolyline : null);
          onEstimate(typeof route?.estimatedTotal === 'number' && Number.isFinite(route.estimatedTotal) ? { estimatedTotal: route.estimatedTotal, durationSeconds: typeof route.durationSeconds === 'number' ? route.durationSeconds : null } : null);
        } })
        .catch(() => { if (!controller.signal.aborted) { setEncodedPolyline(null); onEstimate(null); } });
    }, 350);
